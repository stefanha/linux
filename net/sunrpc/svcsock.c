...
static int svc_vsock_has_wspace(struct svc_xprt *xprt)
{
	...
	if (test_bit(XPT_LISTENER, &xprt->xpt_flags))
		return 1;
	required = atomic_read(&xprt->xpt_reserved) + serv->sv_max_mesg;
	if (vsock_stream_has_space(vsock_sk(svsk->sk_sk)) >= required ||
	    (sk_stream_min_wspace(svsk->sk_sk) == 0 &&
	     atomic_read(&xprt->xpt_reserved) == 0))
		return 1;
	set_bit(SOCK_NOSPACE, &svsk->sk_sock->flags);
...
static void svc_vsock_write_space(struct sock *sk)
{
	struct svc_sock *svsk = (struct svc_sock *)sk->sk_user_data;
	struct socket *sock = sk->sk_socket;
	...
	if (!sk_stream_is_writeable(sk) || !sock)
		return;
	if (!svsk || svc_vsock_has_wspace(&svsk->sk_xprt))
		clear_bit(SOCK_NOSPACE, &sock->flags);
	svc_write_space(sk);
}
...
static struct svc_xprt_ops svc_vsock_ops = {
	...
	.xpo_secure_port = svc_sock_secure_port,
	.xpo_adjust_wspace = svc_vsock_adjust_wspace,
};
...
static void svc_vsock_init(struct svc_sock *svsk, struct svc_serv *serv)
{
	...
	if (sk->sk_state == VSOCK_SS_LISTEN) {
		...
		sk->sk_write_space = svc_vsock_write_space;
