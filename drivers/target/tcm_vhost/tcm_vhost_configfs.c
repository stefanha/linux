...
static ssize_t tcm_vhost_tpg_show_dev_index(
	struct se_portal_group *se_tpg,
	char *page)
{
	struct tcm_vhost_tpg *tv_tpg = container_of(se_tpg,
	...
	return snprintf(page, PAGE_SIZE, "%u\n", tv_nexus->tvn_dev_index);
}

static ssize_t tcm_vhost_tpg_store_dev_index(
	struct se_portal_group *se_tpg,
	const char *page,
	size_t count)
{
	...
	int ret;
	/*
	 * Currently we require the vHost I_T nexus has already been
	 * created for this TPG..
	 */
	mutex_lock(&tv_tpg->tv_tpg_mutex);
	tv_nexus = tv_tpg->tpg_nexus;
	if (!tv_nexus) {
	...
	/*
	 * Remove active virtio_scsi device association if 'NULL' is passed..
	 */
	if (!strncmp(page, "NULL", 4)) {
		...
		/*
		 * Make the call into tcm_vhost_scsi.c code to release
		 * the tv_nexus->tvn_dev_index association to our active
		 * vhost_scsi device.
		 */
		ret = vhost_scsi_dev_index_release(tv_tpg,
					tv_nexus->tvn_dev_index);
		if (!ret) {
	...
	/*
	 * Extract the requested dev_index, and grab a reference to
	 * tv_tpg_vhost_count to prevent the tcm_nexus from being released
	 * below us.
	 */
	...
	smp_mb__after_atomic_inc();
	/*
	 * Make the call into tcm_vhost_scsi.c to associate the
	 * dev_index obtained via VHOST_SCSI_GET_INDEX.
	 */
	ret = vhost_scsi_dev_index_associate(tv_tpg, dev_index);
	if (ret < 0) {
		atomic_dec(&tv_tpg->tv_tpg_vhost_count);
		smp_mb__after_atomic_dec();
	}
...
TF_TPG_BASE_ATTR(tcm_vhost, dev_index, S_IRUGO | S_IWUSR);

static struct configfs_attribute *tcm_vhost_tpg_attrs[] = {
	&tcm_vhost_tpg_nexus.attr,
	&tcm_vhost_tpg_dev_index.attr,
	NULL,
};
