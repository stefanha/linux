...
nfsd_start() {
	...
	*vsock*)
		...
		sudo "$NFSUTILSDIR/mountd/mountd" -d all
		sudo "$NFSUTILSDIR/exportfs/exportfs" -r -d all
		sudo "$NFSUTILSDIR/nfsd/nfsd" -N3 --vsock 2049
		;;
	esac
}
...
nfsd_stop() {
	...
	*vsock*)
	...
	for mod in nfsd nfs_acl lockd grace rpcsec_gss_krb5 auth_rpcgss sunrpc
	do
		rmmod_nofail $mod
	done
}
...
vhost_start() {
	...
	sudo insmod net/vmw_vsock/vmw_vsock_virtio_transport_common.ko dyndbg==p
	sudo insmod drivers/vhost/vhost_vsock.ko dyndbg==p
}

vhost_stop() {
	rmmod_nofail vhost_vsock
	rmmod_nofail vmw_vsock_virtio_transport_common
	rmmod_nofail vsock
	rmmod_nofail vhost
}
